...
<?= $this->Form->create($product, ['url' => ['action' => 'fukuseitempdo']]) ?>
...
?>
      ...
      <?php for($i=1; $i<=${"tuikagenryou".$j}; $i++): ?>
        ...
        <?= $this->Form->control('recycled_mixing_ratio'.$j, array('type'=>'hidden', 'value'=>${"recycled_mixing_ratio".$j.$i}, 'label'=>false)) ?>
      <?php endfor;?>
